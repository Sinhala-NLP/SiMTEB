from simteb.tasks.classification import SOLDClassification


TASK_REGISTRY = {
    "SOLDClassification": SOLDClassification,
}
...
def get_task(name):
    if name not in TASK_REGISTRY:
        raise KeyError(
            f"Unknown task: {name}. "
            f"Available tasks: {list(TASK_REGISTRY)}"
        )
...
def get_tasks(names=None, categories=None):
    ...
    if categories is not None:
        ...
        return [get_task(name) for name in selected_names]
